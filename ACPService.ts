...
type ACPSession = {
  ...
  updatedAt: string;
  fileSystemProviderName?: string | undefined;
  terminalProvider?: ACPTerminalProvider;
  terminalProviderName?: string | undefined;
};
...
export default class ACPService implements TokenRingService {
  ...
  async createSession(params: NewSessionRequest): Promise<NewSessionResponse> {
    ...
    const configuredAgentConfig = configuredAgentType ? agentManager.getAgentConfig(configuredAgentType) : undefined;
    const fallbackAgentType = agentManager.getAgentTypes()[0];
    const baseAgentConfig = configuredAgentConfig ?? (fallbackAgentType ? agentManager.getAgentConfig(fallbackAgentType) : undefined);
    ...
    const agent = agentManager.spawnAgentFromConfig(sessionAgentConfig as ParsedAgentConfig);
  ...
  async prompt(connection: AgentSideConnection, params: PromptRequest): Promise<PromptResponse> {
    const session = this.requireSession(params.sessionId);
    if (session.activePrompt) {
      throw RequestError.invalidRequest({ sessionId: params.sessionId }, `Session ${params.sessionId} already has an active prompt`);
    }

    const { message, attachments } = convertPromptToAgentInput(params.prompt);
    const cursor = session.agent.getState(AgentEventState).getEventCursorFromCurrentPosition();
    ...
    const streamSignal = AbortSignal.any([connection.signal, session.agent.agentShutdownSignal]);
    ...
    try {
      for await (const state of session.agent.subscribeStateAsync(AgentEventState, streamSignal)) {
        for (const event of state.yieldEventsByCursor(cursor)) {
          const emittedAssistantOutput = await this.forwardEvent(connection, session, event, messageIds);
          ...
          if (!sawAssistantOutput || event.status !== "success") {
            await this.emitAssistantText(
              ...
              session.sessionId,
              event.message,
              sawAssistantOutput && event.status !== "success" ? randomUUID() : messageIds.assistant,
            );
          }
  ...
  private async forwardEvent(connection: AgentSideConnection, session: ACPSession, event: AgentEventEnvelope, messageIds: StreamMessageIds): Promise<boolean> {
    switch (event.type) {
      case "output.chat":
  ...
  private async emitAssistantText(connection: AgentSideConnection, sessionId: string, text: string, messageId: string): Promise<void> {
    if (!text.trim()) {
      return;
    }
    await connection.sessionUpdate({
  ...
  private async emitThoughtText(connection: AgentSideConnection, sessionId: string, text: string, messageId: string): Promise<void> {
    if (!text.trim()) {
      return;
    }
    await connection.sessionUpdate({
  ...
  private configureSessionProviders(session: ACPSession): void {
    ...
    if (fileSystemService && (this.clientCapabilities.fs?.readTextFile || this.clientCapabilities.fs?.writeTextFile)) {
      ...
      fileSystemService.registerFileSystemProvider(providerName, new ACPFileSystemProvider(this.connection, session.sessionId, this.clientCapabilities));
